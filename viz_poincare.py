...
def poincare_2d_visualization(model, tree, figure_title, num_nodes=50, show_node_labels=(), color=False, indexs=None):
    ...
    if color == False:
        ...
        assert (indexs != None)
        color1 = "rgb"+str(tuple(int('0165fc'[i:i+2], 16) for i in (0, 2, 4)))
        color2 = "rgb"+str(tuple(int('ff9408'[i:i+2], 16) for i in (0, 2, 4)))
        color3 = "rgb"+str(tuple(int('02ab2e'[i:i+2], 16) for i in (0, 2, 4)))
        color4 = "rgb"+str(tuple(int('e50000'[i:i+2], 16) for i in (0, 2, 4)))
        color5 = "rgb"+str(tuple(int('bf77f6'[i:i+2], 16) for i in (0, 2, 4)))
        color6 = "rgb"+str(tuple(int('653700'[i:i+2], 16) for i in (0, 2, 4)))
        color7 = "rgb"+str(tuple(int('ff81c0'[i:i+2], 16) for i in (0, 2, 4)))
        color8 = "rgb"+str(tuple(int('929591'[i:i+2], 16) for i in (0, 2, 4)))
        color9 = "rgb"+str(tuple(int('e6daa6'[i:i+2], 16) for i in (0, 2, 4)))
        color10 = "rgb"+str(tuple(int('00ffff'[i:i+2], 16) for i in (0, 2, 4)))
        colors = [color1, color2, color3, color4, color5, color6,color7, color8, color9, color10]
        nodes = list()

        for key, i in enumerate(indexs):
            vectors = np.asarray([model.kv.syn0[j] for j in i])
            if vectors.shape[1] != 2:
                raise ValueError('Can only plot 2-D vectors')

            node_labels = [model.kv.index2word[j] for j in i]
            nodes_x = list(vectors[:, 0])
            nodes_y = list(vectors[:, 1])
    ...
    if color == True:
        nodes.append(edges)
        nodes.append(nodes_with_labels)
        return go.Figure(data=nodes, layout=layout)
    else:
